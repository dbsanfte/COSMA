 ...
 * 
 * Tests BF16 data transfers across MPI ranks using cosma communicator functions.
 * This validates that BF16 can be transferred correctly using MPI_UINT16_T.
 * 
...
bool test_mpi_send_receive() {
    ...
    if (size < 2) {
        if (rank == 0) {
            std::cerr << "ERROR: This test requires at least 2 MPI ranks" << std::endl;
        }
        return false;
    }
    ...
    if (rank == 0) {
        ...
        std::cout << "Rank 0: Sent " << N << " BF16 values to Rank 1" << std::endl;
    } 
    else if (rank == 1) {
        MPI_Recv(recv_buffer.data(), N, MPI_UINT16_T, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        ...
        if (passed) {
            std::cout << "Rank 1: Successfully received and verified " << N << " BF16 values" << std::endl;
        }
        return passed;
    }
...
bool test_mpi_broadcast() {
    ...
    if (rank == 0) {
        ...
        std::cout << "Rank 0: Broadcasting " << N << " BF16 values" << std::endl;
    }
    ...
    for (int i = 0; i < N; ++i) {
        ...
        if (std::abs(received - expected) > 1e-6f) {
            std::cerr << "Rank " << rank << ": buffer[" << i << "] = " << received 
                      << " (expected: " << expected << ")" << std::endl;
            passed = false;
        }
    }
    ...
    if (passed && rank != 0) {
        std::cout << "Rank " << rank << ": Successfully received broadcast data" << std::endl;
    }
...
bool test_mpi_allreduce() {
    ...
    MPI_Allreduce(send_fp32.data(), recv_fp32.data(), N, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);
    ...
    if (passed && rank == 0) {
        std::cout << "Allreduce test passed (sum across " << size << " ranks = " 
...
bool test_mpi_type_mapper() {
    ...
    if (bf16_type != MPI_UINT16_T) {
        std::cerr << "ERROR: mpi_mapper<bfloat16>::getType() returned wrong type" << std::endl;
        return false;
    }
    ...
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    if (rank == 0) {
        std::cout << "MPI type mapper test passed (BF16 → MPI_UINT16_T)" << std::endl;
    }
...
int main(int argc, char** argv) {
    ...
    if (rank == 0) {
        std::cout << "===== BFloat16 MPI Communication Tests =====" << std::endl;
        std::cout << std::endl;
    }
    ...
    if (rank == 0) std::cout << "Testing MPI type mapper..." << std::endl;
