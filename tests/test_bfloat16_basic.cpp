...
void test_bf16_conversion() {
    ...
    
    {
        ...
        std::cout << "  π: " << val << " → " << result 
                  << " (error: " << std::abs(result - val) << ") ✓" << std::endl;
    }
...
void test_bf16_gemm_simple() {
    ...
    A[0] = bfloat16(1.0f); A[1] = bfloat16(3.0f);  // First column
    A[2] = bfloat16(2.0f); A[3] = bfloat16(4.0f);  // Second column
    
    // Initialize B (column-major)
    B[0] = bfloat16(5.0f); B[1] = bfloat16(7.0f);  // First column
    B[2] = bfloat16(6.0f); B[3] = bfloat16(8.0f);  // Second column
    ...
    for (int i = 0; i < M * N; ++i) {
        ...
        if (rel_error > 0.02f) {  // 2% tolerance
            std::cerr << "ERROR: Result " << i << " exceeds tolerance!" << std::endl;
            passed = false;
        }
    }
...
void test_bf16_gemm_larger() {
    ...
    gemm(M, N, K, 1.0f, A_fp32.data(), M, B_fp32.data(), K, 0.0f, C_fp32.data(), M);
